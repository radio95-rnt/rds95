...
void encoder_saveToFile(RDSEncoder *enc) {
	...
	tempEncoder.streams = enc->streams;

	RDSEncoderFile rdsEncoderfile = {.file_starter = 225, .file_middle = 160, .file_ender = 95, .program = tempEncoder.program, .streams = tempEncoder.streams};
...
int encoder_loadFromFile(RDSEncoder *enc) {
	...
	enc->streams = rdsEncoderfile.streams;
	return 0;
}